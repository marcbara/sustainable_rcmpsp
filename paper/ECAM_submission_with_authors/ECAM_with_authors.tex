...
%\usepackage{lineno}      % not required for ECAM
...
\renewcommand{\thetable}{\Roman{table}}   % ECAM requires Roman-numeral table numbering
...
\par}
\vspace{1cm}
{\small\textit{Submitted to Engineering, Construction and Architectural Management}}
\vfill
\end{titlepage}
...
in drought-prone regions, yet scheduling models treat water as an unlimited resource.
This paper proposes a multi-objective framework for construction portfolios subject
to stochastic daily water-quota restrictions arising from drought regulations,
enabling managers to quantify and manage water-scarcity risk at portfolio level.

\medskip\noindent\textbf{Design/methodology/approach} --
The bi-objective RCMPSP minimises tardiness cost and daily water-quota violations.
Restriction shocks follow a two-state Markov chain capturing Mediterranean
drought-spell persistence; stochasticity is handled via Sample Average
Approximation~(SAA).  A MOEA/D algorithm is adapted to the RCMPSP with a novel
water-arbitrage local-search operator that explicitly exploits the temporal
structure of drought spells.

\medskip\noindent\textbf{Findings} --
Under mild to moderate drought the algorithm eliminates all tardiness penalties
while reducing water violations by up to~19.4\% relative to a naive baseline.
The Pareto front yields a directly readable marginal cost of compliance for
contract negotiations.  Under severe drought the front collapses, revealing the
threshold at which technological rather than scheduling intervention is required.

\medskip\noindent\textbf{Originality/value} --
This is the first study to embed stochastic daily water-quota constraints into the
RCMPSP and introduce a problem-specific metaheuristic operator exploiting
drought-spell persistence.  Unlike concurrent exact-method approaches
\citep{Kong2025}, the framework scales to realistic portfolio sizes under
uncertainty.  The managerial framework translates Pareto-front outputs into
actionable contract-level decisions.
...
water as an unlimited resource.  This assumption is increasingly untenable across
a wide range of climates: in the Mediterranean basin, regulatory authorities in
Spain, Italy and Greece regularly issue day-specific water quotas that penalise
over-consumption; analogous restrictions have been applied to construction sites
during drought emergencies in California, south-eastern Australia and parts of
sub-Saharan Africa, reflecting a broader global trend toward time-varying
water-use regulation driven by climate change.  For a mid-size construction
portfolio, unplanned quota violations can generate fines of tens of thousands of
euros and trigger cascading delays when water-intensive activities must be
...
capacity or an aggregate sustainability metric.  Unlike labour or equipment
constraints, which are fully under the contractor's control, water quotas are set
externally by regulatory authorities on a day-by-day basis in response to reservoir
levels and hydrological conditions; the scheduler cannot increase the supply side,
only rearrange demand in time.  The scheduling decision must therefore exploit
...
This paper addresses that gap by introducing a bi-objective RCMPSP in which
tardiness penalties and daily water-quota violations are jointly minimised under
stochastic restriction conditions.  Restriction shocks follow a two-state Markov
chain calibrated to Mediterranean drought-spell persistence; uncertainty is handled
via Sample Average Approximation, absorbing the stochastic structure into the problem
data rather than embedding it in the optimiser.  MOEA/D~\citep{Zhang2007} is adapted
to the RCMPSP context through a priority-vector representation decoded by a Serial
Schedule Generation Scheme~(SGS) and augmented with a water-arbitrage local-search
operator that systematically moves water-intensive activities out of restriction
windows by exploiting the temporal persistence of drought spells.  Applying this
framework to a representative Mediterranean construction portfolio shows that the value
of water-aware scheduling is strongly scenario-dependent: the algorithm produces rich
trade-off frontiers under mild and moderate drought — the regime most consistent with
current Iberian projections — while the same front collapses under extreme scarcity,
signalling that violations are then climatically determined and that technological
rather than scheduling interventions are required.  This transition from scheduling
leverage to climate determination is itself a decision-relevant finding that existing
...
Most recently, \citet{Kong2025} proposed an exact three-phase CPLEX procedure with
a quota auction mechanism for dedicated RCMPSP with calendar constraints,
demonstrating strong performance on moderate-sized instances; however, exact methods
become intractable as portfolio size and stochastic elements grow, motivating the
metaheuristic approach developed here.
...
monetary penalties, typically expressed as a fixed fee per cubic metre consumed
above the daily quota.  These penalties accrue on the same accounting horizon as
tardiness costs, making them directly comparable at the contract level.  The
financial framing shifts water from an abstract sustainability metric to a cash-flow
risk item that project managers must actively monitor and minimise alongside schedule
delays, strengthening the case for its explicit inclusion in the project-scheduling
objective function.
...
In the construction domain, \citet{Huang2025} recently applied NSGA-II with
Monte Carlo simulation to schedule-cost trade-offs in high-rise buildings under
uncertainty, confirming the viability of population-based multi-objective methods
for stochastic construction scheduling; however, their work addresses a single
project rather than a portfolio with shared resources.
\citet{Yu2025} applied MOEA/D-DE to safety and
...
Concretely, for the moderate scenario ($\mu_R = 14$, $\mu_F = 21$), a construction
manager on a restriction day can expect approximately 14~more restricted days before
a free window opens; that free window will in turn last around 21~days on average.
These spell lengths are consistent with multi-week drought patterns observed in
Mediterranean climates and give the water-arbitrage operator a meaningful
reallocation target: shifting a water-intensive activity by one to two weeks can
avoid an entire restriction spell.
...
\begin{enumerate}
  \item Identify the construction day $t^*$ with the highest quota violation
        ($t^* = \arg\max_t [W_t - Q_t]^+$, where $W_t$ is actual daily
        water consumption).
...
Because restriction spells are temporally persistent --- typically spanning one to
three weeks --- successfully shifting a water-intensive activity out of a spell
eliminates violations across multiple consecutive days simultaneously.  A random
swap of two priority-vector entries, by contrast, is as likely to move an activity
\emph{into} a restriction window as out of one, providing no systematic pressure
toward compliance.
...
consistent with the sustained high-pressure blocking that drives summer droughts
...
default list order (no water awareness) and decoding through the SGS.  This baseline represents the outcome of a conventional scheduler that
...
\begin{subfigure}[b]{0.65\textwidth}
  \includegraphics[width=\textwidth]{figures/pareto_front_mild.png}
  \caption{Mild scenario ($\pi_R = 20\%$)}
\end{subfigure}

\medskip
\begin{subfigure}[b]{0.65\textwidth}
  \includegraphics[width=\textwidth]{figures/pareto_front_moderate.png}
  \caption{Moderate scenario ($\pi_R = 40\%$)}
\end{subfigure}

\medskip
\begin{subfigure}[b]{0.65\textwidth}
  \includegraphics[width=\textwidth]{figures/pareto_front_severe.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{figures/convergence_mild.png}
...
project manager's decision process.  Operationalising the results follows a natural
sequential logic rooted in the structure of the trade-off.

At project initiation, the team selects the drought scenario consistent with the
climatological outlook for the construction window.  In the absence of
site-specific meteorological forecasts, the moderate scenario provides the
appropriate conservative baseline for contemporary Mediterranean construction.
With the scenario fixed, MOEA/D is run on the scenario-specific restriction
sequence to produce the trade-off curve between tardiness penalties and water
violations.

Selecting a preferred solution requires the project manager to weigh the relative
financial exposures against the shape of the front.  Under a lump-sum contract the
contractor bears the full cost of delays, favouring the minimum-tardiness
solution~A; under a cost-plus arrangement the client shares delay risk, shifting
the balance toward the water-compliant solution~C; solution~B offers a staged
intermediate compromise.  The key analytical input to this decision is the
marginal cost of water compliance — the slope of the Pareto front between adjacent
solutions.  Comparing this marginal schedule cost to the unit regulatory fine
determines whether the water-optimal schedule is economically justified: if the
fine per cubic metre is lower than the marginal cost of achieving it through
rescheduling, solution~A is preferred on financial grounds alone.  The same price
of compliance serves as a technology-procurement threshold — if an on-site
water-recycling system costs less per cubic metre than the marginal schedule
adjustment, capital investment in that technology dominates further schedule
optimisation.

Finally, the framework provides a re-optimisation trigger for mid-project use.
Should climate conditions deviate materially from the planned scenario — an
unexpected prolonged drought, for instance — MOEA/D is re-run on an updated
restriction sequence for the remaining horizon, seeding the initial population
from the current partial schedule.  This closes the loop between planning-phase
optimisation and construction-phase execution.
...
Applied to a representative three-project portfolio (130~activities, 12~resources),
the results demonstrate that water-aware scheduling delivers measurable financial
value — but only within bounds set by the prevailing climate regime.  Under mild
and moderate drought, MOEA/D recovered the full \euro36,500 tardiness penalty that
the naive baseline incurred while simultaneously reducing water violations by
13.3--19.4\%, achieving both objectives without relaxing any contractual deadline.
The Pareto front makes this value financially tangible: the marginal cost of
reducing water violations by one cubic metre — approximately \euro14.85/m$^3$ in
the mild scenario — is directly usable as a threshold in contract negotiations and
as a benchmark for evaluating water-saving technologies.

Under severe drought the picture changes fundamentally.  The Pareto front
degenerates to a single point, not as a computational failure but as a structural
finding: when 60\% of days are under restriction, the construction phases are too
long and the free windows too narrow for any schedule to differentiate itself in
terms of water compliance.  This transition from scheduling leverage to climate
determination is itself a decision-relevant output — one that existing RCMPSP
frameworks, which treat water as a fixed resource or aggregate volume metric,
cannot produce.  Identifying the drought threshold beyond which scheduling
optimisation should give way to greywater recycling, emergency water rights or
rainfall-indexed insurance is therefore a contribution of the present work that
extends beyond the numerical results.

Several directions merit future investigation.  The present study uses a
representative synthetic portfolio calibrated to Iberian construction norms; while
this is standard practice in RCMPSP benchmarking \citep{Hartmann2022}, validation
against real project data from drought-affected regions --- including digitised
contract records from Spain, California or south-eastern Australia --- would
strengthen the empirical grounding of the approach and is a natural next step.
First, the framework currently uses a single set of simulated restriction sequences;
averaging over 10--30 independent sequences would provide confidence intervals on
the Pareto front at the cost of longer computation.  Second, the addition of a resource-levelling
objective would enable a tri-objective formulation; MOEA/D scales naturally to
three objectives by extending the weight vectors to a two-dimensional simplex.
In both cases, the decision-relevant insight lies primarily in the shape of the
trade-off and the existence of regime transitions, rather than in the precise
numerical location of individual Pareto points.

\section*{Acknowledgements}
\noindent The authors have no specific acknowledgements to declare.
...
\medskip\noindent
\textbf{Declaration of conflicting interests:} The authors declare no conflict of interest.

\medskip\noindent
\textbf{Funding:} This research received no external funding.

\medskip\noindent
\textbf{AI usage statement:} Claude~(Sonnet~4.6, Anthropic) was used to
copy-edit portions of the manuscript --- specifically, to improve the clarity
and grammatical structure of existing author-written text --- in accordance with
Emerald's policy on permissible AI assistance.  No AI tool was used to generate
research content, create new material, or produce results.  The authors take
full responsibility for the accuracy, integrity, and originality of the work.
