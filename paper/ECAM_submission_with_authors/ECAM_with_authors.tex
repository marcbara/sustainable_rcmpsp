...
resource.  This paper proposes a multi-objective scheduling framework for
construction project portfolios subject to stochastic daily water-quota restrictions
arising from drought regulations, enabling project managers to quantify and manage
water-scarcity risk at portfolio level.

\medskip\noindent\textbf{Design/methodology/approach} --
The problem is formulated as a bi-objective Resource-Constrained Multi-Project
Scheduling Problem~(RCMPSP) in which the objectives are tardiness cost and daily
water-quota violations.  Daily restriction shocks follow a two-state Markov chain
that captures the persistent drought spells of Mediterranean climates; stochasticity
is handled via Sample Average Approximation~(SAA).  A MOEA/D algorithm is adapted
to the RCMPSP context with a novel water-arbitrage local-search operator that
explicitly exploits the temporal structure of drought spells.

\medskip\noindent\textbf{Findings} --
Under mild to moderate drought the algorithm simultaneously eliminates all tardiness
penalties while reducing water violations by up to~19.4\% relative to a naive
baseline.  The Pareto front provides a directly readable marginal cost of
compliance, giving concrete input for contract negotiations.  Under severe drought
the trade-off curve collapses, revealing the threshold at which scheduling
optimisation loses leverage and technological intervention becomes necessary.

\medskip\noindent\textbf{Originality/value} --
This is the first study to embed stochastic daily water-quota constraints into the
RCMPSP and to introduce a problem-specific metaheuristic operator exploiting the
persistence structure of drought spells.  Unlike concurrent exact-method approaches
to RCMPSP \citep{Kong2025}, the proposed framework scales to realistic portfolio
sizes under uncertainty.  The accompanying managerial framework translates
Pareto-front outputs into actionable contract-level decisions.
...
\begin{enumerate}
  ...
  \item \textbf{Preferred solution selection.}  In cooperation with the client,
        ...
        compromise.  Under a lump-sum contract the contractor bears the full cost
        of delays, favouring solution~A; under a cost-plus arrangement the client
        shares delay risk, shifting the balance toward water compliance.
  ...
  \item \textbf{Price of water compliance.}  Compute the marginal cost of reducing
        ...
        grounds alone.  This price of compliance also serves as a threshold for
        technology procurement: if the per-m$^3$ cost of an on-site water-recycling
        system is lower than the marginal schedule cost, investment in that
        technology dominates further schedule adjustment.
